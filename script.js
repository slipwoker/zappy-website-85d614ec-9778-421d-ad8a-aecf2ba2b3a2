    ...
    if (contactForm) {
        ...
        function validateField(input) {
            ...
            if (input.hasAttribute('required') && value === '') {
                ...
                showError(input, (isRTL ? 'هذا الحقل مطلوب' : fieldName + ' is required'));
                return false;
            }
            ...
            if (input.hasAttribute('minlength') && value.length < parseInt(input.getAttribute('minlength'))) {
                ...
                showError(input, (isRTL ? 'يجب أن يكون ' + minLen + ' أحرف على الأقل' : 'Must be at least ' + minLen + ' characters'));
                return false;
            }

            if (input.hasAttribute('maxlength') && value.length > parseInt(input.getAttribute('maxlength'))) {
                const maxLen = input.getAttribute('maxlength');
                showError(input, (isRTL ? 'يجب ألا يتجاوز ' + maxLen + ' أحرف' : 'Must not exceed ' + maxLen + ' characters'));
                return false;
            }
            ...
            if (input.hasAttribute('pattern')) {
                ...
                if (value !== '' && !pattern.test(value)) {
                    const patternMsg = input.getAttribute('data-pattern-error') || (isRTL ? 'تنسيق غير صالح' : 'Invalid format');
                    showError(input, patternMsg);
                    return false;
                }
            }
        ...
        contactForm.addEventListener('focusout', function(e) {
            ...
            if (input && (input.hasAttribute('required') || input.type === 'email' || input.hasAttribute('minlength') || input.hasAttribute('maxlength') || input.hasAttribute('pattern'))) {
                validateField(input);
            }
        });
        ...
        contactForm.addEventListener('submit', function(e) {
            ...
            inputs.forEach(function(input) {
                if (input.hasAttribute('required') || input.type === 'email' || input.hasAttribute('minlength') || input.hasAttribute('maxlength') || input.hasAttribute('pattern')) {
            ...
            if (isValid) {
                ...
                if (successMessage) {
                    ...
                    successEl.textContent = isRTL ? 'تم إرسال النموذج بنجاح!' : 'Form submitted successfully!';
...
(function() {
    ...
    contactForm.addEventListener('submit', async function(e) {
        ...
        try {
            ...
            var payload = {
                websiteId: '85d614ec-9778-421d-ad8a-aecf2ba2b3a2',
                name: resolvedName,
                email: resolvedEmail,
            ...
            if (result.success) {
                ...
                if (result.thankYouPagePath && result.ticketNumber) {
                    ...
                    if (isPreview && window.ZAPPY_CONFIG) {
                        var wid = window.ZAPPY_CONFIG.websiteId || '85d614ec-9778-421d-ad8a-aecf2ba2b3a2';
...
(function(){
  try {
    ...
    document.addEventListener('submit', function(e) {
      ...
      var wid = '85d614ec-9778-421d-ad8a-aecf2ba2b3a2';
